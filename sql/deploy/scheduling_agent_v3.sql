...
IF OBJECT_ID('[schdl].[ScheduleParameterDispatchConfig]','U') IS NOT NULL DROP TABLE [schdl].[ScheduleParameterDispatchConfig];
IF OBJECT_ID('[schdl].[ParameterDispatchConfig]',        'U') IS NOT NULL DROP TABLE [schdl].[ParameterDispatchConfig];
IF OBJECT_ID('[schdl].[ScheduleDocumentParameter]',      'U') IS NOT NULL DROP TABLE [schdl].[ScheduleDocumentParameter];
IF OBJECT_ID('[schdl].[DocumentParameter]',              'U') IS NOT NULL DROP TABLE [schdl].[DocumentParameter];
IF OBJECT_ID('[schdl].[ScheduleDocument]',               'U') IS NOT NULL DROP TABLE [schdl].[ScheduleDocument];
...
-- 1.2  Schedule definitions
--
--  No FK to a document catalogue -- each schedule owns its own
--  document config via ScheduleDocument (1.3 below).
...
-- 1.3  Schedule-owned document config
--
--  Each schedule owns exactly one document config row.
--  DocumentName is the stable env-agnostic key used to resolve
--  the live documentId at dispatch time via fn_FetchDocumentId.
CREATE TABLE [schdl].[ScheduleDocument] (
    ScheduleDocumentID      INT             IDENTITY(1,1) PRIMARY KEY,
    ...
    CreatedAt               DATETIME2       NOT NULL DEFAULT GETDATE(),
    ModifiedAt              DATETIME2       NOT NULL DEFAULT GETDATE(),
    CONSTRAINT UQ_ScheduleDocument UNIQUE (ScheduleID, DocumentName)
);
GO


-- 1.4  Schedule-owned parameter definitions
--
--  Parameters belong to a schedule, not to a shared document.
--  Two schedules using the same report can have different
--  parameter sets with no cross-schedule interference.
CREATE TABLE [schdl].[ScheduleDocumentParameter] (
    ScheduleParameterID INT             IDENTITY(1,1) PRIMARY KEY,
    ScheduleDocumentID  INT             NOT NULL
        REFERENCES [schdl].[ScheduleDocument](ScheduleDocumentID),
    ...
    CONSTRAINT UQ_ScheduleDocumentParameter UNIQUE (ScheduleID, ParameterName)
);
GO


-- 1.5  Per-parameter dispatch / fan-out config
--
--  One row per parameter that drives fan-out or delivery routing.
--  Scoped to a ScheduleID so two schedules sharing a parameter
--  name never interfere with each other.
--
--  DispatchMode
--    COMBINED    all values bundled into one report, combined destination
--    INDIVIDUAL  one report per value, destination resolved per value
--    BOTH        produces INDIVIDUAL rows AND one COMBINED row
--
--  Source pattern applies to all resolver fields:
--  STATIC       literal value; tokens resolved at dispatch time
--  DYNAMIC_SQL  SELECT with {VALUE} placeholder
CREATE TABLE [schdl].[ScheduleParameterDispatchConfig] (
    ConfigID                INT             IDENTITY(1,1) PRIMARY KEY,
    ScheduleParameterID     INT             NOT NULL
        REFERENCES [schdl].[ScheduleDocumentParameter](ScheduleParameterID),
    ScheduleID              INT             NOT NULL
        REFERENCES [schdl].[Schedule](ScheduleID),
    ...
    -- Per-entity folder resolver (NULL inherits Schedule.FolderSourceValue)
    FolderSource            NVARCHAR(20)    NULL
        CHECK (FolderSource IN ('STATIC','DYNAMIC_SQL')),
    FolderSourceValue       NVARCHAR(MAX)   NULL,

    -- Per-entity subject override (NULL falls back to Schedule.SubjectSourceValue)
    SubjectSource           NVARCHAR(20)    NULL
        CHECK (SubjectSource IN ('STATIC','DYNAMIC_SQL')),
    SubjectSourceValue      NVARCHAR(MAX)   NULL,

    -- Per-entity body override (NULL falls back to Schedule.BodySourceValue)
    BodySource              NVARCHAR(20)    NULL
        CHECK (BodySource IN ('STATIC','DYNAMIC_SQL')),
    BodySourceValue         NVARCHAR(MAX)   NULL,

    CONSTRAINT UQ_ScheduleParamDispatch UNIQUE (ScheduleID, ScheduleParameterID)
);
GO


-- 1.6  Schedule parameter values
--
...
--     Executed at dispatch time -- results joined as pipe-delimited
--     values, exactly as if typed in ParameterValue.
--     Example:
...
CREATE TABLE [schdl].[ScheduleParameter] (
    ...
    ScheduleParameterID  INT             NOT NULL REFERENCES [schdl].[ScheduleDocumentParameter](ScheduleParameterID),
    ParameterValue       NVARCHAR(MAX)   NOT NULL,
    ParameterValueQuery  NVARCHAR(MAX)   NULL,
    CONSTRAINT UQ_ScheduleParam UNIQUE (ScheduleID, ScheduleParameterID)
);
GO


-- 1.7  Standing CC/BCC recipients
--
--  Static email addresses included on every delivery for this schedule.
--  The main TO recipient lives on Schedule.EmailSourceValue.
--  These are CC/BCC only -- never TO.
...
-- 1.9  Dispatch queue  (one row per delivery action)
...
--    {{TODAY-N}}  dynamic negative offset, any N
--    {{TODAY+N}}  dynamic positive offset, any N
...
--  Use in usp_BuildDispatchQueue to resolve FolderPath, FileName,
...
--
--  Resolves the API documentId for a given ScheduleID.
--  Looks up DocumentName from ScheduleDocument, then queries
--  the live document catalogue.
--
--  IMPLEMENT ONCE PER ENVIRONMENT -- replace the SELECT body
--  with the query that fetches documentId from your source.
--
--  Input  : @ScheduleID  INT
...
--
--  Flow:
--    1. Parse @DispatchJson into schedule-level delivery variables
--    2. UPSERT Schedule                          -> @ScheduleID
--    3. UPSERT ScheduleDocument                  -> @DocID
--    4. Loop @ParametersJson:
--         UPSERT ScheduleDocumentParameter       -> @pParamID
--         UPSERT/DELETE ScheduleParameterDispatchConfig
--         UPSERT ScheduleParameter
--    5. DELETE + re-INSERT ScheduleStandingRecipient
--    6. Return summary row
--
--  PARAMETER QUICK REFERENCE
--  @DocumentName       Stable name -- drives live documentId lookup
...
--  @StartDate          default 2000-01-01
--  @EndDate            NULL = no expiry
--
--  @DispatchJson       Schedule-level delivery config:
--  {
--    "deliveryMethod":      "EMAIL | FOLDER | BOTH",
--    "emailSource":         "STATIC | DYNAMIC_SQL",
--    "emailSourceValue":    "address or SELECT",
--    "subjectSource":       "STATIC | DYNAMIC_SQL",
--    "subjectSourceValue":  "subject or SELECT",
--    "bodySource":          "STATIC | DYNAMIC_SQL",
--    "bodySourceValue":     "body or SELECT",
...
--    "sortOrder":  1,
--    "value":      "BRM001|BRM002"
--    "valueQuery": "SELECT v AS [Value] FROM ..."
--
--    "fanOut": {                   OPTIONAL -- only on primary dispatch parameter
--      "isPrimary":              true,
...
--  }
--
--  @RecipientsJson     CC/BCC standing recipients:
--  [{"email":"x@x.com","role":"CC|BCC","includeInFanOut":false}]
-- ============================================================
CREATE PROCEDURE [schdl].[usp_RegisterSchedule]
    ...
    @StartDate          DATE            = NULL,
    @EndDate            DATE            = NULL,
    @DispatchJson       NVARCHAR(MAX)   = NULL,
...
BEGIN
    ...
    IF @sFileNameSource IS NULL AND JSON_VALUE(@DispatchJson,'$.fileNameTemplate') IS NOT NULL
    BEGIN
        SET @sFileNameSource      = 'STATIC';
        SET @sFileNameSourceValue = JSON_VALUE(@DispatchJson,'$.fileNameTemplate');
    END;
...
--  Builds DispatchQueue rows for one schedule execution.
...
BEGIN
    ...
    IF @sSubjectSource = 'STATIC'
        SET @EmailSubject = @sSubjectSourceValue;
    ELSE IF @sSubjectSource = 'DYNAMIC_SQL' AND ISNULL(@sSubjectSourceValue,'') <> ''
    BEGIN
        SET @subjSQL = N'SELECT TOP 1 @s = Subject FROM (' + @sSubjectSourceValue + N') AS _q';
        EXEC sp_executesql @subjSQL, N'@s NVARCHAR(500) OUTPUT', @s = @EmailSubject OUTPUT;
    END;
    IF @EmailSubject IS NOT NULL
        SET @EmailSubject = [schdl].[fn_ResolveAllTokens](@EmailSubject, @Today, @DocumentName);

    -- Resolve schedule-level body
    IF @sBodySource = 'STATIC'
        SET @EmailBody = @sBodySourceValue;
    ELSE IF @sBodySource = 'DYNAMIC_SQL' AND ISNULL(@sBodySourceValue,'') <> ''
    BEGIN
        SET @bodySQL = N'SELECT TOP 1 @b = Body FROM (' + @sBodySourceValue + N') AS _q';
        EXEC sp_executesql @bodySQL, N'@b NVARCHAR(MAX) OUTPUT', @b = @EmailBody OUTPUT;
    END;
    ...
    DECLARE dv_cur CURSOR LOCAL FAST_FORWARD FOR
        SELECT sp.ScheduleParameterID, sp.ParameterValueQuery, sp.ParameterValue
    ...
    OPEN dv_cur; FETCH NEXT FROM dv_cur INTO @dvSPID, @dvQuery, @dvStatic;
    ...
    BEGIN
        ...
        BEGIN CATCH
            ...
            VALUES (@ScheduleID, 'SKIPPED',
                'ParameterValueQuery failed for ScheduleParameterID '
                + CAST(@dvSPID AS NVARCHAR) + ': ' + ERROR_MESSAGE());
        END CATCH;
        INSERT INTO #DynVals (ScheduleParameterID, ResolvedValue)
        VALUES (@dvSPID, ISNULL(@dvResult, @dvStatic));
        FETCH NEXT FROM dv_cur INTO @dvSPID, @dvQuery, @dvStatic;
    END;
    ...
    INTO #P
    FROM  #Raw
    GROUP BY ScheduleParameterID, ParameterName, DataType, IsRequired, SortOrder,
             IsPrimary, DispatchMode,
             EmailSource, EmailSourceValue,
             DisplayNameSource, DisplayNameSourceValue,
             FileNameSource, FileNameSourceValue,
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                SET @npDynSQL = N'SELECT TOP 1 @e = EmailAddress FROM (' + @sEmailSourceValue + N') AS _q';
        ...
        BEGIN
            ...
            EXEC sp_executesql @npDynSQL, N'@e NVARCHAR(500) OUTPUT', @e = @npFileName OUTPUT;
        END;
        ...
        VALUES
            (@LogID,@ScheduleID,'COMBINED',@sDeliveryMethod,NULL,
             ...
             ISNULL(@npTo,''),@CcAll,@BccAll,@EmailSubject,@EmailBody,@npFolder);
        RETURN;
    END;
    ...
    DECLARE
        ...
        @PrimFolderSrc    NVARCHAR(20),   @PrimFolderSrcVal  NVARCHAR(MAX),
        @PrimSubjectSrc   NVARCHAR(20),   @PrimSubjectSrcVal NVARCHAR(MAX),
        ...
        BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    EXEC sp_executesql @iDynSQL, N'@e NVARCHAR(500) OUTPUT', @e = @iDisplayName OUTPUT;
                END;
            END;
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    EXEC sp_executesql @iDynSQL, N'@e NVARCHAR(1000) OUTPUT', @e = @iFolderPath OUTPUT;
                END;
            END;
            ...
            BEGIN
                ...
                BEGIN
                    SET @iDynSQL = N'SELECT TOP 1 @e = Subject FROM ('
                        + REPLACE(@PrimSubjectSrcVal,'{VALUE}',@iSafeVal) + N') AS _q';
                    EXEC sp_executesql @iDynSQL, N'@e NVARCHAR(500) OUTPUT', @e = @iSubject OUTPUT;
                END;
            END;
            SET @iSubject = ISNULL(@iSubject, @EmailSubject);
            IF @iSubject IS NOT NULL
            BEGIN
            ...
            BEGIN
                ...
                END;
            END;
            SET @iBody = ISNULL(@iBody, @EmailBody);
            IF @iBody IS NOT NULL
            BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            EXEC sp_executesql @bDynSQL, N'@e NVARCHAR(500) OUTPUT', @e = @bFileName OUTPUT;
        END;
        ...
        VALUES
            (@LogID,@ScheduleID,'COMBINED',@sDeliveryMethod,NULL,
             ...
             ISNULL(@bTo,''),@CcAll,@BccAll,@EmailSubject,@EmailBody,@bFolder);
    END;
END;
GO
...
BEGIN
    ...
    -- Diagnostic: gate analysis per active schedule
    SELECT
        s.ScheduleID,
        s.ScheduleName,
        s.FrequencyType,
...
BEGIN
    ...
    UPDATE el
    SET    el.Status = CASE
               WHEN EXISTS (SELECT 1 FROM [schdl].[DispatchQueue] d2
...
--  WITHOUT checking any gates (IsActive, RunTime, NextRunAt, etc.).
--  Does NOT advance NextRunAt or alter IsActive.
--  Cleans up inserted rows unless @KeepResults = 1.
--
--  Usage:
--    EXEC schdl.usp_TestDispatch @ScheduleName = 'My Schedule';
--    EXEC schdl.usp_TestDispatch @ScheduleName = 'My Schedule', @KeepResults = 1;
...
--
--  Reads a registered schedule and reconstructs the exact EXEC call
--  needed to re-register it. Used by the HTML builder load panel.
--
--  Returns one row: ScheduleName, DocumentName, DispatchJson,
--  ParametersJson, RecipientsJson, RegisterSQL.
--
--  Usage:
--    EXEC schdl.usp_GetScheduleJson @ScheduleName = 'My Schedule';
...
BEGIN
    ...
    DECLARE
        ...
        @pSubSrc      NVARCHAR(20),  @pSubSrcVal    NVARCHAR(MAX),
        @pBodSrc      NVARCHAR(20),  @pBodSrcVal    NVARCHAR(MAX),
        @pFirst       BIT = 1,
        @pChunk       NVARCHAR(MAX);
    ...
    FETCH NEXT FROM pc INTO
        ...
        @pSubSrc, @pSubSrcVal, @pBodSrc, @pBodSrcVal;
    ...
    BEGIN
        ...
        BEGIN
            ...
            IF @pSubSrc IS NOT NULL
                SET @pChunk += ',"subjectSource":"'          + STRING_ESCAPE(@pSubSrc,'json')     + '"';
            IF @pSubSrcVal IS NOT NULL
                SET @pChunk += ',"subjectSourceValue":"'     + STRING_ESCAPE(@pSubSrcVal,'json')  + '"';
            IF @pBodSrc IS NOT NULL
                SET @pChunk += ',"bodySource":"'             + STRING_ESCAPE(@pBodSrc,'json')     + '"';
            IF @pBodSrcVal IS NOT NULL
                SET @pChunk += ',"bodySourceValue":"'        + STRING_ESCAPE(@pBodSrcVal,'json')  + '"';
            SET @pChunk += '}';
        END;
...
/*
-- ============================================================
--  SECTION 5  SAMPLE REGISTRATIONS
--  One EXEC per schedule. Everything in one call.
--  Subject and body travel inside @DispatchJson as
--  subjectSource/subjectSourceValue and bodySource/bodySourceValue.
-- ============================================================

-- A: BRM Production Report — BOTH mode (individual per BRM + one combined)
--    Email per BRM resolved via DYNAMIC_SQL.
EXEC [schdl].[usp_RegisterSchedule]
    @DocumentName   = N'BRM Production Report',
    @ReportEndpoint = N'/api/reports/generate',
    @OutputFormat   = 'xlsx',
    @ScheduleName   = N'BRM Production Report - Monthly',
    ...
    @DispatchJson   = N'{
        ...
        "emailSource":       "STATIC",
        "emailSourceValue":  "reports-bulk@example.com",
        "subjectSource":     "STATIC",
        "subjectSourceValue":"BRM Production Report - {{PREV_MONTH_START}} to {{PREV_MONTH_END}}",
        "bodySource":        "STATIC",
        "bodySourceValue":   "Please find attached your production report for the previous month."
    }',
    @ParametersJson = N'[
        {
            ...
            "fanOut":{
                ...
                "emailSourceValue":"SELECT ContactEmail AS EmailAddress FROM dbo.BRMContacts WHERE BRMCode = ''{VALUE}''",
                "displayNameSource":"DYNAMIC_SQL",
                "displayNameSourceValue":"SELECT FullName AS DisplayName FROM dbo.BRMContacts WHERE BRMCode = ''{VALUE}''",
                "fileNameSource":"STATIC",
                "fileNameSourceValue":"BRM_{{DISPLAYNAME}}_{{PREV_MONTH_END}}.xlsx"
            }
        },
...
-- B: Daily Exception Report — combined delivery, STATIC email
EXEC [schdl].[usp_RegisterSchedule]
    @DocumentName   = N'Daily Exception Report',
    @ReportEndpoint = N'/api/reports/generate',
    @ScheduleName   = N'Daily Exception Report - 07:00',
    @FrequencyType  = 'DAILY',
    @RunTime        = '07:00',
    @DispatchJson   = N'{
        "deliveryMethod":    "EMAIL",
        "emailSource":       "STATIC",
        "emailSourceValue":  "it-alerts@example.com",
        "subjectSource":     "STATIC",
        "subjectSourceValue":"Daily Exception Report - {{TODAY}}",
        "bodySource":        "STATIC",
        "bodySourceValue":   "Attached is the exception report for {{TODAY-1}}."
    }',
    @ParametersJson = N'[
        {"name":"AsOfDate","type":"date","required":true,"sortOrder":1,"value":"{{TODAY-1}}"}
    ]';
GO


-- C: Broker Statement — INDIVIDUAL, folder drop + email, path resolved per entity
EXEC [schdl].[usp_RegisterSchedule]
    @DocumentName   = N'Broker Monthly Statement',
    @ReportEndpoint = N'/api/reports/generate',
    @ScheduleName   = N'Broker Monthly Statement - Monthly',
    @FrequencyType  = 'MONTHLY',
    @RunTime        = '05:30',
    @DayOfMonth     = 1,
    @DispatchJson   = N'{
        "deliveryMethod":    "BOTH",
        "emailSource":       "STATIC",
        "emailSourceValue":  "statements@example.com",
        "subjectSource":     "STATIC",
        "subjectSourceValue":"Your Statement - {{PREV_MONTH_START}} to {{PREV_MONTH_END}}",
        "bodySource":        "STATIC",
        "bodySourceValue":   "Please find attached your monthly statement.",
        "folderSource":      "STATIC",
        "folderSourceValue": "\\\\server\\reports\\statements"
    }',
    @ParametersJson = N'[
        {
            "name":"BrokerageID","type":"string","required":true,"sortOrder":1,
            "value":"39398|38|39399",
            "fanOut":{
                "isPrimary":true,"mode":"INDIVIDUAL",
                "emailSource":"DYNAMIC_SQL",
                "emailSourceValue":"SELECT ContactEmail AS EmailAddress FROM dbo.Brokerage WHERE BrokerageID = ''{VALUE}''",
                "folderSource":"DYNAMIC_SQL",
                "folderSourceValue":"SELECT FolderPath FROM dbo.BrokerageReportFolders WHERE BrokerageID = ''{VALUE}''"
            }
        },
...
-- D: Admin Summary — INDIVIDUAL, dynamic SQL email, weekly
EXEC [schdl].[usp_RegisterSchedule]
    @DocumentName   = N'Administrator Summary',
    @ReportEndpoint = N'/api/reports/generate',
    @ScheduleName   = N'Administrator Summary - Weekly',
    ...
    @DispatchJson   = N'{
        "deliveryMethod":    "EMAIL",
        "emailSource":       "STATIC",
        "emailSourceValue":  "admin-combined@example.com",
        "subjectSource":     "STATIC",
        "subjectSourceValue":"Weekly Admin Summary - {{PREV_WEEK_START}} to {{PREV_WEEK_END}}",
        "bodySource":        "STATIC",
        "bodySourceValue":   "Please find attached the weekly administrator summary."
    }',
    @ParametersJson = N'[
        {
...
-- ============================================================
--  SECTION 6  ADMIN REFERENCE QUERIES
-- ============================================================

-- All schedules with resolved documentId
-- SELECT s.ScheduleID, s.ScheduleName, d.DocumentName,
--        s.FrequencyType, s.NextRunAt, s.IsActive,
--        schdl.fn_FetchDocumentId(s.ScheduleID) AS ResolvedDocumentId
-- FROM   schdl.Schedule s
-- JOIN   schdl.ScheduleDocument d ON d.ScheduleID = s.ScheduleID
-- ORDER BY s.NextRunAt;

-- All schedules with parameter values
-- SELECT d.DocumentName, s.ScheduleName, dp.ParameterName,
...
-- All dispatch configs
-- SELECT d.DocumentName, s.ScheduleName, dp.ParameterName,
--        dc.IsPrimaryDispatchKey, dc.DispatchMode,
--        dc.EmailSource, dc.EmailSourceValue
-- FROM   schdl.ScheduleParameterDispatchConfig dc
-- JOIN   schdl.ScheduleDocumentParameter dp ON dp.ScheduleParameterID = dc.ScheduleParameterID
-- JOIN   schdl.Schedule                   s  ON s.ScheduleID          = dc.ScheduleID
-- JOIN   schdl.ScheduleDocument           d  ON d.ScheduleID          = s.ScheduleID
-- ORDER BY d.DocumentName, dp.SortOrder;

-- Standing recipients
-- SELECT s.ScheduleName, r.EmailAddress, r.RecipientRole, r.IncludeInFanOut
-- FROM   schdl.ScheduleStandingRecipient r
-- JOIN   schdl.Schedule s ON s.ScheduleID = r.ScheduleID
-- ORDER BY s.ScheduleName, r.RecipientRole;

-- Recent dispatch queue
-- SELECT TOP 50
--        dq.QueueID, s.ScheduleName, d.DocumentName,
--        dq.DispatchType, dq.DispatchKeyValue,
--        dq.ToAddresses, dq.Status, dq.CreatedAt, dq.ErrorMessage
-- FROM   schdl.DispatchQueue  dq
-- JOIN   schdl.Schedule       s  ON s.ScheduleID = dq.ScheduleID
-- JOIN   schdl.ScheduleDocument d ON d.ScheduleID = s.ScheduleID
...
-- Reload schedule into HTML builder
...
--     Result set 1: gate diagnostics (all active schedules)
--     Result set 2: PENDING DispatchQueue rows to process
...
--        Subject=EmailSubject Body=EmailBody Attachment=report from (a)
--        (skip if DeliveryMethod=FOLDER)
--     c. Drop file to FolderPath (skip if DeliveryMethod=EMAIL)
--     d. EXEC schdl.usp_UpdateDispatchStatus
